package com.mimisalon.casestudy.controller;
        ...
        import com.mimisalon.casestudy.database.dao.CustomerDAO;
        import com.mimisalon.casestudy.database.entity.Booking;
        import com.mimisalon.casestudy.database.entity.Customer;
        import com.mimisalon.casestudy.database.entity.User;
        import com.mimisalon.casestudy.form.CreateCustomerFormBean;
...
public class CustomerController {
    ...
    public ModelAndView detail(@PathVariable Integer customerId,@PathVariable String customerName) {
        ...
        if (customer == null) {
            log.warn("Customer with id " + customerId + " was not found");
            // In a real application, you might redirect to a 404 page here because the customer was not found
            response.setViewName("redirect:/error/404");
        } else {
            // Retrieve booking data related to the customer
            List<Booking> bookings =bookingDAO.findByCustomerName(customerName);
            response.addObject("customer", customer);
            response.addObject("bookings", bookings);
        }
